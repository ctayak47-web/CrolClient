...
import crol.client.utility.render.display.Render2DUtil;
...
import crol.client.utility.render.display.shader.BlurProgram;
...
implements IWindow {
    ...
    public static void initializeShaders() {
        rectangleProgram = new GlProgram(CrolClient.id("rectangle/data"), VertexFormats.POSITION_COLOR);
        squircleProgram = new GlProgram(CrolClient.id("squircle/data"), VertexFormats.POSITION_COLOR);
        squircleTextureProgram = new GlProgram(CrolClient.id("squircle_texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
        roundedTextureProgram = new GlProgram(CrolClient.id("texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
        borderProgram = new GlProgram(CrolClient.id("border/data"), VertexFormats.POSITION_COLOR);
        figmaBorderProgram = new GlProgram(CrolClient.id("corner/data"), VertexFormats.POSITION_COLOR);
        loadingProgram = new GlProgram(CrolClient.id("loading/data"), VertexFormats.POSITION_COLOR);
        gradientRectangleProgram = new GlProgram(CrolClient.id("gradient_rectangle/data"), VertexFormats.POSITION_COLOR);
    ...
    public static void drawRoundedCorner(MatrixStack matrices, float x, float y, float width, float height, float borderThikenes, float delta, ColorRGBA color, BorderRadius radius) {
        if (!CrolClient.getInstance().getThemeManager().getCurrentTheme().isCorners()) {
            return;
        }
    ...
    public static void drawBlurHud(MatrixStack matrices, float x, float y, float width, float height, float blurRadius, BorderRadius borderRadius, ColorRGBA color) {
        boolean blur = CrolClient.getInstance().getThemeManager().getCurrentTheme().isBlur();
        boolean glow = CrolClient.getInstance().getThemeManager().getCurrentTheme().isGlow();
    ...
    public static void drawGlow(MatrixStack matrixStack, float x, float y, float width, float height, int glowRadius) {
        Render2DUtil.drawGradientBlurredShadow(matrixStack, x, y, width, height, glowRadius, CrolClient.getInstance().getThemeManager().getClientColor());
    }
