...
import crol.client.utility.math.IntRange;
...
extends RotationMode {
    ...
    public Rotation process(InterpolationRotationConfig config, Rotation modelOut, Rotation targetRotation) {
        Pair<Float, Float> pair = this.calculateFactors(CrolClient.getInstance().getRotationManager().getCurrentRotation(), targetRotation, config.getHorizontalSpeedSetting(), config.getVerticalSpeedSetting(), config.getDirectionChangeFactor(), config.getMidPoint());
    ...
    public Pair<Float, Float> calculateFactors(Rotation currentRotation, Rotation targetRotation, IntRange horizontalSpeedSetting, IntRange verticalSpeedSetting, IntRange directionChangeFactor, float midpoint) {
        ...
        if (targetRotation != null && CrolClient.getInstance().getRotationManager().getPreviousRotationTarget() != null) {
            directionChange = CrolClient.getInstance().getRotationManager().getPreviousRotationTarget().targetRotation().angleTo(targetRotation);
