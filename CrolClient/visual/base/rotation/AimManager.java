...
import crol.client.base.rotation.mods.AIRotationMode;
import crol.client.base.rotation.mods.InstantRotationMode;
import crol.client.base.rotation.mods.InterpolationRotationMode;
import crol.client.base.rotation.mods.config.AiRotationConfig;
import crol.client.base.rotation.mods.config.InstantRotationConfig;
import crol.client.base.rotation.mods.config.InterpolationRotationConfig;
...
implements IClient {
    ...
    public Rotation rotate(RotationConfig config, Rotation targetRotation) {
        if (config.getType() != RotationModeType.INSTANT) {
            RotationDelta deltaToTarget = CrolClient.getInstance().getRotationManager().getCurrentRotation().rotationDeltaTo(targetRotation);
        ...
        Rotation newRotation = switch (config.getType()) {
            ...
            case RotationModeType.INTERPOLATION -> this.interpolationMod.process((InterpolationRotationConfig)config, CrolClient.getInstance().getRotationManager().getCurrentRotation(), targetRotation);
            ...
            default -> CrolClient.getInstance().getRotationManager().getCurrentRotation();
        };
