...
package crol.client.screens.menu.panels;
...
import crol.client.screens.menu.panels.SideBarCategory;
import crol.client.utility.math.MathUtil;
import crol.client.utility.render.display.base.BorderRadius;
import crol.client.utility.render.display.base.GuiUtil;
...
public class SidebarPanel {
    ...
    public void render(UIContext ctx, float boxX, float boxY, float height, float progress, Theme theme, Category selectedCategory, ColorRGBA primary, ColorRGBA textColor, ColorRGBA selectedColor) {
        ...
        ctx.drawText(logoGlyphFont, logoText, logoTextX, logoTextY, CrolClient.getInstance().getThemeManager().getColorCycleIcon().toGradient().mulAlpha(progress));
        ctx.pushMatrix();
        ...
        DrawUtil.drawRoundedTexture(ctx.getMatrices(), CrolClient.id("icons/avatar.png"), avatarX, avatarY, avatarSize, avatarSize, BorderRadius.all(4.0f), ColorRGBA.WHITE.mulAlpha(progress));
