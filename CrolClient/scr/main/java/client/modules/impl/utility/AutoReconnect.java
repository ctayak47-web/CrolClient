...
extends Module {
    ...
    private int resolveCurrentAnarchy() {
        int trackedAnarchy;
        if (CrolClient.getInstance().getServerHandler() != null && (trackedAnarchy = CrolClient.getInstance().getServerHandler().getAnarchy()) > 0) {
    ...
    private boolean isSupportedServer() {
        if (CrolClient.getInstance().getServerHandler() != null && CrolClient.getInstance().getServerHandler().isCopyTime()) {
            return true;
        }
