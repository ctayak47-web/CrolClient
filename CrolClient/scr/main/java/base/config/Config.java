...
import crol.client.utility.culling.EntityCullingManager;
...
public class Config {
    ...
    public JsonObject save() {
        try {
            ...
            for (Module module : CrolClient.getInstance().getModuleManager().getModules()) {
                modulesObject.add(module.getName(), module.save());
            }
            root.add("Modules", modulesObject);
            ThemeManager themeManager = CrolClient.getInstance().getThemeManager();
            JsonObject themeObject = new JsonObject();
            themeObject.addProperty("selected", themeManager.getCurrentTheme().getName());
            themeObject.addProperty("columns", CrolClient.getInstance().getMenuScreen().getColumns());
    ...
    public void load(JsonObject object) {
        if (object.has("Theme")) {
            ...
            if (themeObject.has("selected")) {
                ...
                CrolClient.getInstance().getThemeManager().switchThemeByName(selected);
            }
            if (themeObject.has("columns")) {
                int columns = themeObject.get("columns").getAsInt();
                CrolClient.getInstance().getMenuScreen().setColumns(columns);
            }
            ThemeManager themeManager = CrolClient.getInstance().getThemeManager();
