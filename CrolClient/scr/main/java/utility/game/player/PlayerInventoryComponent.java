...
import crol.client.utility.game.player.MovingUtil;
...
implements IMinecraft {
    ...
    public static void addTask(Runnable task) {
        if (MovingUtil.hasPlayerMovement()) {
            CrolClient.getInstance().getScriptManager().addTask(script);
            switch (CrolClient.getInstance().getServerHandler().getServer()) {
                case "FunTime": 
                case "HolyWorld": {
