package ru.crolclient.common.util.math;
...
import ru.crolclient.mixins.accessors.GameRendererAccessor;
...
public class ProjectionUtil implements QuickImports {
    public static Vector2f project(double x, double y, double z) {
        ...
        float fov = (float)((GameRendererAccessor) mc.gameRenderer)
                .invokeGetFov(camera, mc.getRenderTickCounter().getTickDelta(true), true);
    ...
    private static void calculateViewBobbing(PlayerEntity playerEntity, Vector3f result3f) {
        ...
        if (playerEntity instanceof AbstractClientPlayerEntity clientPlayer) {
            f = clientPlayer.distanceMoved - clientPlayer.lastDistanceMoved;
            f1 = -(clientPlayer.distanceMoved + f * mc.getRenderTickCounter().getTickDelta(true));
        }
        float f2 = MathHelper.lerp(mc.getRenderTickCounter().getTickDelta(true), playerEntity.prevStrideDistance, playerEntity.strideDistance);
