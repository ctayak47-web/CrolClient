...
import vurst.visual.base.comand.impl.args.CfgArgumentType;
...
extends CommandAbstract {
    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(((LiteralArgumentBuilder)ConfigCommand.literal("save").then(ConfigCommand.arg("name", StringArgumentType.word()).executes(context -> {
            VurstVisual.getInstance().getConfigManager();
            String name = ConfigManager.normalizeConfigName(StringArgumentType.getString((CommandContext)context, (String)"name"));
            boolean success = VurstVisual.getInstance().getConfigManager().saveConfig(name);
            if (success) {
            ...
            boolean success = VurstVisual.getInstance().getConfigManager().saveConfig("current_config");
            if (success) {
        ...
        builder.then(((LiteralArgumentBuilder)ConfigCommand.literal("load").then(ConfigCommand.arg("name", CfgArgumentType.create()).executes(context -> {
            ...
            boolean success = VurstVisual.getInstance().getConfigManager().loadConfig(name);
            if (success) {
            ...
            boolean success = VurstVisual.getInstance().getConfigManager().loadConfig("current_config");
            if (success) {
