...
package vurst.visual.utility.game.other;
...
implements IMinecraft {
    public static final Text PREFIX = (Text)Text.of((String)"[ %s ] ".formatted("Vurst Visual")).copy().append(Text.of((String)"★")).getWithStyle(Style.EMPTY.withColor(ColorRGBA.BLUE.getRGB())).getFirst();

    public static void displayMessage(LogLevel level, String message) {
        if (MessageUtil.mc.player == null) {
            return;
        }
        Text icon = (Text)Text.of((String)"[ %s ] ".formatted("Vurst Visual")).copy().append(Text.of((String)"★")).getWithStyle(Style.EMPTY.withColor(VurstVisual.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB())).getFirst();
