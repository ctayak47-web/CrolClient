...
extends Module {
    ...
    public boolean shouldBlockHubCommand(String command) {
        ...
        if (PvpSave.mc.player != null) {
            PvpSave.mc.player.sendMessage((Text)Text.literal((String)"(Vurst Visual) ").formatted(Formatting.BLUE).append((Text)Text.literal((String)"Вы точно хотите выйти с PvP режима? Пропишите еще раз /hub")), false);
        }
        return true;
    }
    ...
    private boolean isPvpEndState(String name) {
        ...
        if (VurstVisual.getInstance().getServerHandler() != null && VurstVisual.getInstance().getServerHandler().isPvpEnd()) {
            return true;
        }
